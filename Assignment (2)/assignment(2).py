...
# *********************** The Canny operator as a multi-step algorithm *********************** #
...
def gaussian_smoothing(input_image):
    ...
    cv2.imwrite(os.path.join(output_path, "gaussian_smoothing.bmp"), convoluted_gaussian_arr)
    return convoluted_gaussian_arr

# ******************************************************************************************************************** #

# step 2: gradient calculation function which takes in the gaussian convoluted matrix as argument
def gradient_operation(smoothed_image):
    ...
    cv2.imwrite(os.path.join(output_path, "horizontal_gradient.bmp"), horizontal_gradient)
    cv2.imwrite(os.path.join(output_path, "vertical_gradient.bmp"), vertical_gradient)
    ...
    cv2.imwrite(os.path.join(output_path, "gradient_image.bmp"), gradient_mag)
...
def non_maxima_suppression(gradient_mag, gradient_angle):
    ...
    cv2.imwrite(os.path.join(output_path, "non-maxima_suppressed.bmp"), suppressed_arr)
...
def double_thresholding(suppressed_image, gradient_angle):
    ...
    cv2.imwrite(os.path.join(output_path, "double_thresholding.bmp"), output_image)

# ******************************************************************************************************************** #

if __name__ == "__main__":
    # input name of image with extensions on which you want to apply Canny Edge Detection
    image = input("Please Enter image name: ")
    canny(image)
...
# The Canny operator
cany_operator = cv2.Canny(img,100,200)
cv2.imwrite(os.path.join(output_path, "Cany_Operator.bmp"), cany_operator)
...
sobel = sobelx + sobely
cv2.imwrite(os.path.join(output_path, "Sobel_Edge_Detector.bmp"), sobel)
...
cv2.imwrite(os.path.join(output_path, "Prewitt_Gradient_Operator.bmp"), prewittx + prewitty)

# Laplacian of Gaussian
laplacian = cv2.Laplacian(img, cv2.CV_64F)
cv2.imwrite(os.path.join(output_path, "Laplacian_of_Gaussian.bmp"), laplacian)
